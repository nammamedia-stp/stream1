  ...
  <script>
    (function() {
      ...
      function safePlay(videoEl) {
        ...
        console.log('[StreamPulse][VIDEO] Attempting video.play() (muted=' + videoEl.muted + ', readyState=' + videoEl.readyState + ')');
        try {
          const playPromise = videoEl.play();
          if (playPromise !== undefined) {
            return playPromise.then(() => {
              console.log('[StreamPulse][VIDEO] play() resolved successfully.');
              return true;
            }).catch((err) => {
              console.warn('[StreamPulse][VIDEO] play() rejected:', err ? (err.message || err) : 'unknown', 'Retrying with explicit muted state...');
              videoEl.muted = true;
              return videoEl.play().then(() => {
                console.log('[StreamPulse][VIDEO] play() retry succeeded.');
                return true;
              }).catch((e2) => {
                console.warn('[StreamPulse][VIDEO] play() retry failed:', e2 ? (e2.message || e2) : 'unknown');
                return false;
              });
            });
          }
        } catch (e) {
          console.warn('[StreamPulse][VIDEO] Synchronous error during play():', e);
      ...
      function destroyLivePlayer() {
        ...
        if (hlsInstance) {
          try {
            console.log('[StreamPulse][HLS] Destroying HLS instance.');
        ...
        if (liveVideo) {
          ...
          try {
            ...
            liveVideo.onloadeddata = null;
            liveVideo.oncanplay = null;
            liveVideo.ontimeupdate = null;
            liveVideo.pause();
      ...
      function switchToLiveHls(validHlsUrl, newStreamKey) {
        ...
        // Attach event handlers to liveVideo element for multi-phase visual activation
        liveVideo.onplaying = function() {
          console.log('[StreamPulse][VIDEO] onplaying event fired.');
          activateLiveDisplay();
        };
        liveVideo.onloadeddata = function() {
          console.log('[StreamPulse][VIDEO] onloadeddata event fired.');
          safePlay(liveVideo);
          if (liveVideo.currentTime > 0 || liveVideo.readyState >= 2) {
            activateLiveDisplay();
          }
        };
        liveVideo.oncanplay = function() {
          console.log('[StreamPulse][VIDEO] oncanplay event fired.');
          safePlay(liveVideo);
        };
        ...
        if (window.Hls && window.Hls.isSupported()) {
          ...
          hlsInstance.on(window.Hls.Events.MEDIA_ATTACHED, function() {
            console.log('[StreamPulse][HLS] MEDIA_ATTACHED event fired.');
            safePlay(liveVideo);
          });
          ...
          hlsInstance.on(window.Hls.Events.MANIFEST_PARSED, function(event, data) {
            ...
            console.log('[StreamPulse][HLS] MANIFEST_PARSED event fired (levels: ' + levelCount + '). Triggering safePlay.');
            safePlay(liveVideo);
          });

          hlsInstance.on(window.Hls.Events.FRAG_BUFFERED, function(event, data) {
            hlsNetworkRetries = 0; // Reset retries on successful fragment buffering
            if (liveVideo.paused || liveVideo.currentTime === 0) {
              safePlay(liveVideo);
            }
            if (liveVideo.currentTime > 0) {
              activateLiveDisplay();
            }
          });
            ...
            if (data && data.fatal) {
              ...
              if (data.type === window.Hls.ErrorTypes.MEDIA_ERROR) {
                ...
                try {
                  ...
                  console.error('[StreamPulse][ERROR] Media error recovery failed:', e);
                ...
                if (hlsNetworkRetries <= MAX_HLS_NETWORK_RETRIES) {
                  ...
                  try {
                    ...
                    console.error('[StreamPulse][ERROR] Error during startLoad retry:', e);
                  }
                } else {
                  console.warn('[StreamPulse][ERROR] Network retries exhausted. Stream is offline or unreachable.');
                  switchToOfflineStandby('Stream Endpoint Network Unreachable');
                }
              } else {
                console.error('[StreamPulse][ERROR] Unhandled fatal HLS error. Destroying engine and switching to Standby.');
          ...
          console.log('[StreamPulse][HLS] Native HLS supported by browser. Setting liveVideo.src =', cacheBustUrl);
          liveVideo.src = cacheBustUrl;
          safePlay(liveVideo);
        } else {
          console.error('[StreamPulse][ERROR] HLS.js is not supported and native HLS is missing.');
      ...
      async function runPollCycle() {
        ...
        try {
          ...
          console.error('[StreamPulse][ERROR] Exception during runPollCycle:', e);
          consecutiveOfflineCycles++;
        }
